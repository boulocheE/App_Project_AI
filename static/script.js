...
function affichageImage(event, numPhoto) {
	const imgIn  = document.getElementById("imgInSortie" );
	const imgOut = document.getElementById("imgOutSortie");

	if ( numPhoto == 1 ) {
		imgOut.style.display = "none";
		imgIn .style.display = "bloc";
	} else {
		imgIn .style.display = "none";
		imgOut.style.display = "bloc";
	}

	window.open( "../templates/pages/visualisationPhoto.html", "Photo visualisations", "whidth = 500, height = 500" );
}